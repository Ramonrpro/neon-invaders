...
import { CANVAS_CENTER_Y, CANVAS_HEIGHT, CENTER_X, LOGICAL_WIDTH } from '@game/config/screen';
...
export function ensureCrtTexture(scene: Phaser.Scene): void {
  ...
  const texture = scene.textures.createCanvas(CRT_TEXTURE, LOGICAL_WIDTH, CANVAS_HEIGHT);
  if (!texture) return;
  ...
  for (let y = 0; y < CANVAS_HEIGHT; y += CRT.scanlinePeriodPx) {
  ...
  const outerRadius = Math.hypot(CENTER_X, CANVAS_CENTER_Y);
  ...
  ctx.fillRect(0, 0, LOGICAL_WIDTH, CANVAS_HEIGHT);
